...
import ReportCarousel from "../../components/ReportCarousel/ReportCarousel";
...
export default function ResultsPage() {
    ...
    function loadDocuments(indexOf, count = 10, remaining = 10) {
        ...
        Publication.getDocuments(publicationsList.slice(indexOf, indexOf + count).map(x => x.encodedId))
            .then(response => {
                ...
                {(loadedDocs.length > 0 && !isError) &&
                    <section className={styles.documents}>
                        ...
                        <div className={styles.documents__wrapper}>
                            {loadedDocs.map(x => 
                                <DocumentPage data={x.ok} key={x.ok.id} />
                            )}
                        </div>
                        ...
                        <div className={styles.buttons}>
                            ...
                            <ArrowUp className={remainingPublications > 0 ? styles.up_button : styles.up_button__only} onClick={onUpBtnClick} />
                        </div>
                    </section>
                }
